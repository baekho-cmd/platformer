...
function showWin(){
  // show menu with congrats
  const menu = document.getElementById('menu');
  menu.style.display = 'flex';
  menu.querySelector('.subtitle').textContent = '축하합니다! 모든 스테이지 클리어!';
...
function loop(){
  // Only update player physics during play
  if(window.gameState === 'playing'){
    player.handle(); player.update(platforms);

    // fall death: below view
  ...
  let camX = Math.max(0, player.x + player.w/2 - W/2);
  ...
  for(let p of platforms) ctx.fillRect(p.x-camX,p.y,p.w,p.h);

  // walls
  if(walls && walls.length){ ctx.fillStyle = '#8b6f47'; for(let w of walls) ctx.fillRect(w.x-camX, w.y, w.w, w.h); }

  // finish
  if(finishRect){ ctx.fillStyle = '#ffd27a'; ctx.fillRect(finishRect.x-camX, finishRect.y, finishRect.w, finishRect.h); }

  // red (dead) blocks
  if(redBlocks && redBlocks.length){ ctx.fillStyle = '#d94b4b'; for(let r of redBlocks) ctx.fillRect(r.x-camX, r.y, r.w, r.h); }

  // player
  player.draw(camX);

  // HUD
  const stageInfo = `Stage: ${currentStage+1}/${stages.length}`;
  document.getElementById('info').textContent = `${stageInfo}`;
